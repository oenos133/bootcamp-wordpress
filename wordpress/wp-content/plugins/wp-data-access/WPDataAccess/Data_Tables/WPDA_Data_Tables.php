...
class WPDA_Data_Tables
{
    ...
    )
    {
        ...
        return $wpda_wpdataaccess_prepare_filter . "<table id=\"" . esc_attr( $table_name ) . "{$pub_id}\" class=\"display\" cellspacing=\"0\">" . "<thead>" . $this->show_header(
            ...
            $buttons,
            ''
        ) . "</tfoot>" . "</table>" . "<script type='text/javascript'>" . "var datatables_i18n_url = '" . plugins_url( '../assets/i18n/', __DIR__ ) . "';" . "var {$columnsvar} = [" . $wpda_database_columns . "];" . "jQuery(function () {" . "\twpda_datatables_ajax_call(" . "\t\t" . $columnsvar . "," . "\t\t\"" . esc_attr( $database ) . "\"," . "\t\t\"" . esc_attr( $table_name ) . "\"," . "\t\t\"" . esc_attr( $column_names ) . "\"," . "\t\t\"" . esc_attr( $responsive ) . "\"," . "\t\t\"" . esc_attr( $responsive_popup_title ) . "\"," . "\t\t\"" . esc_attr( $responsive_type ) . "\"," . "\t\t\"" . esc_attr( $responsive_icon ) . "\"," . "\t\t\"" . esc_attr( $language ) . "\"," . "\t\t\"" . htmlentities( $sql_orderby ) . "\"," . "\t\t" . $pub_table_options_searching . "," . "\t    " . $pub_table_options_ordering . "," . "\t\t" . $pub_table_options_paging . "," . "\t\t\"" . esc_attr( $pub_table_options_advanced ) . "\"," . "\t\t" . $pub_id . "," . "\t\t\"" . esc_attr( $pub_responsive_modal_hyperlinks ) . "\"," . "\t\t[" . implode( ',', $hyperlink_positions ) . "]," . "\t\t\"" . esc_attr( $filter_field_name ) . "\"," . "\t\t\"" . esc_attr( $filter_field_value ) . "\"," . "\t\t\"" . esc_attr( $nl2br ) . "\"," . "\t\t" . $buttons . "" . "\t);" . "});" . "</script>";
    }
